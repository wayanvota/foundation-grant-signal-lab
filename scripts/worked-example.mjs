...
export async function buildWorkedExample(target) {
  ...
  const exclusion = reportArtifact("exclusion_report", ruleSpec, { results: evaluation.results, clauseSummary: evaluation.clauseSummary, clauseFrequency: evaluation.clauseFrequency, clauseFrequencyFindings: evaluation.clauseFrequencyFindings, impactReport: impact }, { generatedAt });
  const selfScreen = reportArtifact("self_screen", ruleSpec, generateSelfScreen(ruleSpec), { generatedAt });
  const admittedCount = evaluation.results.filter((item) => item.outcome.status === "ADMIT").length;
  const funnel = reportArtifact("funnel_projection", ruleSpec, calculateFunnel({}, { admittedCount }), { generatedAt });
...
function renderPage({ ruleSpec, exclusionReport, selfScreen, funnelProjection }) {
  ...
  const frequencyFindings = exclusionReport.clauseFrequencyFindings.map((item) => `<article class="evidence-card"><h3>${escape(item.clauseId)}</h3><p>${escape(item.message)}</p></article>`).join("");
  const impact = exclusionReport.impactReport;
  const impactMarkup = impact.computed
    ? `<div class="metric-grid"><article class="metric-card"><small>Filing-thin exclusion rate (n=${impact.filingThin.count})</small><strong>${percent(impact.filingThin.exclusionRate)}</strong></article><article class="metric-card"><small>Other profiles (n=${impact.other.count})</small><strong>${percent(impact.other.exclusionRate)}</strong></article><article class="metric-card"><small>Difference</small><strong>${impact.differentialPercentagePoints} points</strong></article></div>${impact.attribution ? `<p>${escape(impact.attribution)}</p>` : ""}`
    : `<p class="boundary">${escape(impact.message)}</p>`;
...
<section class="example-section"><p class="eyebrow">Retrospective view</p><h2>Clause frequency</h2><div class="table-wrap"><table><thead><tr><th>Clause</th><th>Would exclude</th><th>Missing fact</th><th>Source language</th></tr></thead><tbody>${frequencies}</tbody></table></div><div class="card-stack">${frequencyFindings}</div></section>
<section class="example-section"><p class="eyebrow">Filing-thin comparison</p><h2>Rule Impact Report</h2>${impactMarkup}</section>
